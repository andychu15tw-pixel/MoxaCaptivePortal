...
### 10.1 為何「Moxa-only」架構沒有 CoA 問題，移到公網才有？

**Q: RADIUS server 放 Moxa box 內時，daloRADIUS 踢人按一下就通，CoA 沒這 silent-drop 問題。為什麼?**

**A: 因為原本路徑是 loopback，沒走網路。**

| 架構 | CoA 路徑 | chilli 反應 |
|------|----------|-------------|
| **Moxa-only (原)** | daloRADIUS PHP @ Moxa → `radclient 127.0.0.1:3799` → chilli loopback | ACK ✅ |
| **公網 (改)** | daloRADIUS PHP @ 10.90.35.47 → `radclient 10.90.35.36:3799` → chilli WAN | silent drop ❌ |

```
[原本]  daloRADIUS @ Moxa
         ↓ shell_exec radclient
        127.0.0.1:3799              ← loopback，封包經 lo 接口進 chilli
         ↓
        chilli                       ← 接受並 ACK
```

```
[改公網]  daloRADIUS @ 10.90.35.47
           ↓ shell_exec radclient
          10.90.35.36:3799           ← 經 eth0 進 chilli
           ↓
          chilli silent drop          ← chilli 1.6 internal 拒
```

### 10.2 為何 chilli 對非 loopback CoA 挑剔

chilli 1.6 source 內 CoA handler 對 source IP 有額外驗證，即使設定 `coanoipcheck = 1` 也未能完全跳過（具體原因須查 source code）。實測行為一致：

- source IP **127.0.0.1** → ACK 正常
- source IP **10.90.35.47**（即使等於 `radiusserver1` 設定值）→ silent drop（無 reply、無 chilli journal log）
- 加 `Message-Authenticator` 屬性也無法繞過

驗證指令（在 Moxa 上跑）：
```bash
# OK：loopback
echo User-Name=testuser | radclient 127.0.0.1:3799 disconnect $SECRET
# → Disconnect-ACK

# FAIL：從公網送同樣的 packet
ssh public-server "echo User-Name=testuser | radclient 10.90.35.36:3799 disconnect $SECRET"
# → No reply from server
```

tcpdump 證實封包到達 Moxa eth0，chilli 收到但不處理。

### 10.3 SSH wrapper 的設計動機

既然 chilli 只接 loopback CoA，把 radclient 動作搬回 Moxa 本機跑：

```
daloRADIUS PHP @ 10.90.35.47
  ↓ shell_exec
ssh moxa@10.90.35.36 (forced-command, key-based)
  ↓
/usr/local/sbin/coa-disconnect.sh <username>
  ↓
radclient 127.0.0.1:3799            ← 又變 loopback
  ↓
chilli ACK ✅
```

**安全考量：**
- SSH key 限定 forced-command（`command="..."` in `authorized_keys`），無法執行其他指令
- wrapper script 對 username 做 regex validation `^[A-Za-z0-9_.@-]+$` 防 injection
- sudoers 只允許 `radclient` + `cat secrets.env`，無 root shell
...
| 架構 | CoA | 集中管理 | 適用 |
|------|-----|---------|------|
| **Moxa-only** | loopback 直通，原生支援 | 多台 Moxa 各自管 user | 單台、PoC、低需求 |
| **公網 RADIUS + SSH wrapper** | 多一跳 SSH，但通 | 一個 daloRADIUS 管多 Moxa | 多 site、企業部署 |
| **公網 RADIUS + WireGuard** (v2) | tunnel 內 RADIUS，加密 | 同上 + 安全 | 跨 Internet 部署 |

如果只有一台 Moxa 且不打算擴展，**Moxa-only 最簡單可靠**。但凡需要多 site 共用 user pool，公網方案就必要 — 加 SSH wrapper 避開 chilli 1.6 的 quirk。

### 10.5 已知 chilli 1.6 CoA 限制摘要

| 限制 | 解法 |
|------|------|
| 必須含 `User-Name`（單獨 `Acct-Session-Id` / `Calling-Station-Id` silent drop） | radclient command 一定加 `User-Name` |
| 非 loopback source 即使 IP 對也 silent drop | SSH wrapper 走 loopback |
| 不主動 flush conntrack（kicked client 舊連線仍通） | `condown` hook 自動 flush /24 |
| 不主動斷 Wi-Fi（L2 association 由 AP 管） | 接受視覺上 Wi-Fi 仍連、實際無網；或整合 AP API 送 deauth |
